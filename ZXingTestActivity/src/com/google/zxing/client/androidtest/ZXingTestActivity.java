...
public final class ZXingTestActivity extends Activity {
	...
	private static final String PACKAGE_NAME = ZXingTestActivity.class
			.getPackage().getName();
	...
	public void onCreate(Bundle icicle) {
		...
		findViewById(R.id.get_camera_parameters).setOnClickListener(
				getCameraParameters);
		...
		findViewById(R.id.search_book_contents).setOnClickListener(
				searchBookContents);
		...
		findViewById(R.id.encode_hidden_data).setOnClickListener(
				encodeHiddenData);
		findViewById(R.id.encode_bad_data).setOnClickListener(encodeBadData);
		findViewById(R.id.share_via_barcode)
				.setOnClickListener(shareViaBarcode);
	...
	private final View.OnClickListener getCameraParameters = new View.OnClickListener() {
		...
		public void onClick(View v) {
			String stats = CameraConfigurationUtils
					.collectStats(getFlattenedParams());
			writeStats(stats);
	...
	private final View.OnClickListener runBenchmark = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			intent.setClassName(ZXingTestActivity.this,
					BenchmarkActivity.class.getName());
			startActivity(intent);
		}
	};
	...
	private static void writeStats(String resultString) {
		File cameraParamsFile = new File(Environment
				.getExternalStorageDirectory().getPath()
				+ "/CameraParameters.txt");
		Writer out = null;
		try {
			out = new OutputStreamWriter(
					new FileOutputStream(cameraParamsFile),
